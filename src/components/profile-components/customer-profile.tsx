...
export const CustomerProfilePage = () => {
  ...
  useEffect(() => {
    const fetchTrainers = async () => {
      try {
        const response = await axios.get(`/Customer/PersonalTrainers/${auth?.id}`);
        ...
        <div className="w-full max-w-6xl flex flex-col lg:flex-row gap-6">
          ...
          <Card className="w-full lg:w-1/4 p-6 bg-background border border-default-200 dark:border-default-100 shadow-lg rounded-xl flex flex-col items-center">
            <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-foreground">
              {user?.profilePicture ? (
                <img alt={user.fullName} className="w-full h-full object-cover" src={user.profilePicture} />
              ) : (
                <div className="w-full h-full flex items-center justify-center bg-gray-700 text-gray-300">No Image</div>
              )}
            </div>
          ...
          <Card className="flex-1 p-6 bg-background border border-default-200 dark:border-default-100 shadow-lg rounded-xl">
            ...
            <Select className="mb-4 w-1/2" value={selectedExercise} onChange={(e) => setSelectedExercise(e.target.value)}>
